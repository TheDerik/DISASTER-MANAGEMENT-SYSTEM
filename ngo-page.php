...
if (!isset($_SESSION['ngo-loggedin']) || $_SESSION['ngo-loggedin'] != true) {
...
$stmt1 = $conn->prepare("SELECT * FROM `ngo` WHERE `email` = ?");
$stmt1->bind_param("s", $_SESSION['ngo-email']);
$stmt1->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $stmt2 = $conn->prepare("SELECT * FROM `ngo` WHERE `email` = ?");
    ...
    $stmt_insertdetails = $conn->prepare("INSERT INTO `ngo-msgs` (`name`, `email`, `message`) VALUES (?, ?, ?)");
...
<body>
    ...
    <div class="contact-content font1 max-width-1 m-auto">
        <div class="max-width-1 m-auto mx-1">
            ...
            <form action="ngo-page.php?email=<?php echo $_SESSION['ngo-email']; ?>" method="post" enctype="multipart/form-data">
                <div class="contact-form">
                    <div class="form-box">
                        <textarea name="msg" id="" cols="30" rows="10" placeholder="How may we help you?"></textarea>
                        <input type="hidden" name="email" value="<?php echo $_SESSION['ngo-email']; ?>">
                    </div>
                    <div class="form-box">
